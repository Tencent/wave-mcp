...
信号分组名是个例外，它画在 Surfer 的 WASM 画布里，那份字体图集只含 `Ubuntu-Light` 和
`NotoEmoji-Regular`，没有 CJK 字形，所以写中文会显示成方块（分组本身照常生效，只是标题
不可读）。这不是配置能绕过的：WASM 对 JS 只暴露 `start(canvas)`，没有任何字体注入接口，
`egui 0.35` 也没有编译进系统字体 fallback，要真正支持中文分组名得改 Surfer 源码重新编译。

因此**分组名请用简短的 ASCII 词**，四个 demo 就是这么做的（`clock_reset`、`datapath`、
`fast_domain`、`crc_divergence`）。名字里的空格由服务端自动转成下划线：sucl 解析器不接受
带空格的参数，原样发过去整个分组标题会被静默丢弃。
