...
| 2 | `find_instances` | 两级层次：`uart_top.u_tx` → `u_baud_gen` |
...
| 10 | `files(modules_of=)` | 单文件 3 个模块 |
| 11 | `signal_values` | 无波形时正确返回 "needs waveform" 提示 |

## 文件

- `uart_top.sv`：UART TX 设计（顶层 + TX 核 + 波特率分频，两级层次、完整驱动关系）
- `run_demo.py`：一键演示脚本（生成的 session 在 `session/`，已 gitignore）

## 下一步

仿真产出波形后，用同一份 RTL 源码调 `prepare_session` 即可升级为完整 session，
已建好的网表直接复用，值查询 / trace 类工具随即可用。本示例为了把产物留在示例目录里
显式传了 `out_dir`；实际使用中通常省略它，session 会落到 `~/.wave-mcp/sessions/`
下按输入身份命名的目录，静态与波形 session 自动共用网表。
